...
int main(int, char**)
{
    ...
    while (!done)
    {
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            if (ImGui::Button("SelectFile"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
            {
                file_path = IGGetFile();
            }
            if (!file_path.empty()) {
                ImGui::Text("SelectFile [%s]", file_path.c_str());
            }
            if (ImGui::Button("SelectDir"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
            {
                dir_path = IGGetDirectory();
            }
            if (!dir_path.empty()) {
                ImGui::Text("SelectDir [%s]", dir_path.c_str());
            }
